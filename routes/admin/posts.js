...
router.get('/', (req,res,next) => {
	MongoClient.connect(url, (err,db) => {
		if (err) throw err;
		let posts = db.collection('posts');	//  创建存放文章集合
		posts.find().toArray((err,result) => {
			if (err) throw err;
			//  从posts集合中查找到集合，把找到的数据渲染在posts_list表中
...
router.get('/add', (req,res,next) => {
	...
	MongoClient.connect(url,(err,db)=>{
		...
		cats.find().toArray((err,result1)=>{
			...
			tags.find().toArray((err,result2)=>{	//  results1是cats分类查找的结果，result2是tags标签集合的结果,暴露{cats:result1, tags:result2}给前台页面
